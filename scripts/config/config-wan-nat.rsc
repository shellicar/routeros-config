...
:local configureNAT do={
  ...
  :if ($natId = "") do={
    /ip firewall nat add chain="srcnat" out-interface="combo1-WAN" action="masquerade" comment="NAT masquerade for WAN interface"
    $putSuccess "Added NAT masquerade rule for combo1-WAN"
  } else={
    /ip firewall nat set $natId comment="NAT masquerade for WAN interface"
    $putSuccess "Updated NAT masquerade rule for combo1-WAN"
  }
  :put ""
