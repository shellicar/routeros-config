...
:global unconfigureNAT do={
    ...
    :local natCount [:len [/ip firewall nat find where chain="srcnat" and out-interface="combo1-WAN" and action="masquerade"]]
    :if ($natCount > 0) do={
        /ip firewall nat remove [find where chain="srcnat" and out-interface="combo1-WAN" and action="masquerade"]
        :put "Removed NAT masquerade rule from combo1-WAN"
        :log info "Removed NAT masquerade rule from combo1-WAN"
    } else={
        :put "No NAT masquerade rule found on combo1-WAN, skipping"
        :log debug "No NAT masquerade rule found on combo1-WAN, skipping"
    }
    :put ""
